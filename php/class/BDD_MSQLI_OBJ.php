...
class BDD_MSQLI extends BDDAbstract implements BDD {
    ...
    function db_connexion() {
        try {
            $dbAccess = mysqli($this->dbParams['host'], $this->dbParams['user'], $this->dbParams['password'], $this->dbParams['database']);
        } catch (Exception $e) {
    ...
    public function test_connexion() {
        if ($this->connexion) return $this->connexion->ping();
		else return false;
    }
	...
	public function db_query($sqlQuery) {
        ...
        try {
            $query_id = $this->connexion->query($sqlQuery);
